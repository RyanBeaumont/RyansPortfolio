...
50.times do |i|
 student =Student.create!(
   ...
   last_name: "Last #{i + 1}",
   student_email: "student#{i + 1}@msudenver.edu",
  ...
  # Generate a unique profile pic based on the student's name
   avatar_url = "https://robohash.org/#{student.first_name.gsub(' ', '')}"
   avatar = URI.open(avatar_url)
   student.avatar.attach(io: avatar, filename: "#{student.first_name}.jpg")
end
